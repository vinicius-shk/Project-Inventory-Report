...
def test_decorar_relatorio():
    test = ColoredReport(SimpleReport)
    print(test.generate(data))
    resp = test.generate(data).split('\n')
    print('resp', resp)
    new_list = []
    for item in resp:
        prov = item.split(': ')
        print('prov', prov)
        new_list.append(prov[0])
        new_list.append(prov[1])
    print('achaeadna', new_list)
    for index, string in enumerate(new_list):
        if index == len(new_list):
            assert '[31m' in string
            assert '[0m' in string
            break
        if index % 2 != 0:
            assert '[32m' in string
            assert '[0m' in string
        else:
            assert '[36m' in string
            assert '[0m' in string
